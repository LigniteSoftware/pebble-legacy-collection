...
void fctx_init_context_bw(FContext* fctx, GContext* gctx) {
    ...
    if (frameBuffer) {
        ...
        APP_LOG(APP_LOG_LEVEL_INFO, "Creating blank with %d", heap_bytes_free());

        fctx->flag_buffer = gbitmap_create_blank(fctx->flag_bounds.size, GBitmapFormat1Bit);
        bool result = CHECK(fctx->flag_buffer);
        ...
        if(result){
            ...
            APP_LOG(APP_LOG_LEVEL_WARNING, "Failed.");
        }
    }
    else{
        APP_LOG(APP_LOG_LEVEL_WARNING, "Framebuffer is null, what the fuck");
    }
}
