...
void process_tuple(Tuple *t){
    ...
    switch (key) {
        ...
        case APP_KEY_BTDISALERT:
            ...
            break;
        case APP_KEY_TIMEZONE_2:
            data_framework_local_settings.alt_timezone = value;
            break;
        case APP_KEY_SUBTRACT_HOUR:
            data_framework_local_settings.subtract_hour = value;
            break;
        case APP_KEY_NAME_1:
        case APP_KEY_NAME_2:
            strcpy(data_framework_local_settings.names[(key == APP_KEY_NAME_2) ? 1 : 0], t->value->cstring);
            break;
        ...
        case APP_KEY_COLOUR_2:
            data_framework_local_settings.colour[(key == APP_KEY_COLOUR_2) ? 1 : 0] = GColorFromHEX(data_framework_hex_string_to_uint(t->value->cstring));
            break;
        #endif
        case APP_KEY_ANALOGUE_1:
        case APP_KEY_ANALOGUE_2:
            data_framework_local_settings.analogue[(key == APP_KEY_ANALOGUE_2) ? 1 : 0] = value;
            break;
    }
}
...
Settings data_framework_get_default_settings(){
    ...
    defaults.analogue[0] = true;
    defaults.analogue[1] = true;
    strncpy(defaults.names[0], "You", sizeof "You");
    strncpy(defaults.names[1], "Other", sizeof "Other");
    #ifdef PBL_BW
        defaults.colour[0] = GColorWhite;
        defaults.colour[1] = GColorWhite;
    #else
        defaults.colour[0] = GColorRed;
        defaults.colour[1] = GColorOrange;
    #endif
    defaults.alt_timezone = 3600;
    defaults.subtract_hour = false;

    strcpy(defaults.dateFormat[0], "%d/%m/%y");
...
Settings data_framework_port_user_from_settings_1(){
    ...
    newSettings.analogue[0] = oldSettings.analogue[0];
    newSettings.analogue[1] = oldSettings.analogue[1];
    strcpy(newSettings.names[0], oldSettings.names[0]);
    strcpy(newSettings.names[1], oldSettings.names[1]);
    newSettings.colour[0] = oldSettings.colour[0];
    newSettings.colour[1] = oldSettings.colour[1];
    newSettings.alt_timezone = oldSettings.alt_timezone;
    newSettings.subtract_hour = oldSettings.subtract_hour;

    strcpy(newSettings.dateFormat[0], "%d/%m/%y");
