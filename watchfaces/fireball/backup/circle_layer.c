...
void circle_update_travel_path_invert(void *c){
	...
	if(!update_count){
		circle_travel_timers[circle->count] = app_timer_register(50+(1/circle->animation_count)/* 1/x! It's a radical! */, circle_update_travel_path_invert, circle);
	}
	if(update_count){
		circle->count--;
		circle_travel_timers[circle->count] = app_timer_register(15+(1/circle->animation_count), circle_update_travel_path_invert, circle);
	}
}
...
void circle_layer_set_tm(struct tm *t, TimeUnits units){
	if(t->tm_min == 0 && units == 7){
		for(int i = 0; i < 60; i++){
			circle_travel(minute_circles[i], 60, i*5, false);
		}
...
int circle_get_position(int minute, bool x){
	...
	if(x){
		result = (sin_lookup(TRIG_MAX_ANGLE*fixedMinute/60) * (int32_t)CIRCLE_RADIUS / TRIG_MAX_RATIO) + 74;
	}
	else{
		result = (-cos_lookup(TRIG_MAX_ANGLE*fixedMinute/60) * (int32_t)CIRCLE_RADIUS / TRIG_MAX_RATIO) + 78;
	}
	return result;
}
...
int battery_circle_get_position(int percentage, bool x){
	...
	if(x){
		result = (sin_lookup(TRIG_MAX_ANGLE*fixedPercentage/100) * (int32_t)BATTERY_CIRCLE_RADIUS / TRIG_MAX_RATIO) + 76;
	}
	else{
		result = (-cos_lookup(TRIG_MAX_ANGLE*fixedPercentage/100) * (int32_t)BATTERY_CIRCLE_RADIUS / TRIG_MAX_RATIO) + 78;
	}
	return result;
}
